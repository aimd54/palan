# Serving moci-packed models on Kubernetes
...
1. **Init-container puller** — works on any cluster, today. A distroless
   moci image runs `moci pull REF --output /models` into an `emptyDir`;
   the main container is any llama-server image pointed at the file.
   moci brings digest verification, resume, and (later) signature
...
image pull secret mounted as the Docker config — moci reads the Docker
credentials store.

## GPU nodes

The examples are CPU-shaped. On GPU nodes add the device resource and an
accelerated llama-server image (or a moci runtime artifact with a CUDA
flavor):
