...
// testKeyPassword protects the generated cosign-format key; both moci and
...
// encrypted SIGSTORE private key (accepted by cosign and moci alike) plus
...
func TestSignVerifyAndPullGate(t *testing.T) {
	...
	home := t.TempDir()
	moci(t, home, "pack", fx.ggufPath, "-t", signedRef)
	moci(t, home, "push", signedRef)
	moci(t, home, "pack", fx.ggufPath, "-t", unsignedRef)
	moci(t, home, "push", unsignedRef)

	moci(t, home, "sign", signedRef, "--key", priv)
	out := moci(t, home, "verify", signedRef, "--key", pub)
	...
	homeB := t.TempDir()
	moci(t, homeB, "pull", signedRef, "--verify", "--verify-key", pub)
	if out, err := mociRun(homeB, "pull", unsignedRef, "--verify", "--verify-key", pub); err == nil {
	...
	if out, err := mociRun(homeB, "pull", signedRef, "--verify", "--verify-key", otherPub); err == nil {
		t.Errorf("pull verified with the wrong key:\n%s", out)
	}
}

// TestCosignInterop: the real cosign binary must verify moci's signatures
// and moci must verify cosign's (skipped when cosign is not installed).
func TestCosignInterop(t *testing.T) {
	...
	// moci signs → cosign verifies.
	moci(t, home, "sign", ref, "--key", priv)
	...
	if out, err := cv.CombinedOutput(); err != nil {
		t.Errorf("cosign could not verify moci's signature: %v\n%s", err, out)
	}

	// cosign signs → moci verifies (distinct content so the signature tags
	// do not collide with v1's).
	fx2 := writeFixtures(t, 130<<10)
	ref2 := host + "/llm/cosign-interop:v2"
	moci(t, home, "pack", fx2.ggufPath, "-t", ref2)
	moci(t, home, "push", ref2)
	...
	moci(t, home, "verify", ref2, "--key", pub)
}
