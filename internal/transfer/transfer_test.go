...
func seedRegistryModel(t *testing.T, reg *testRegistry, repo, tag string, weights []byte) (mDesc, wDesc ocispec.Descriptor) {
	t.Helper()
...
func TestPullResumesAfterConnectionDrop(t *testing.T) {
	...
	reg.mu.Lock()
	reg.failBlobReads[wDesc.Digest] = 1
	reg.failAfterBytes = 300 * 1024
	reg.mu.Unlock()
...
func TestPushRoundTrip(t *testing.T) {
	...
	reg.mu.Lock()
	_, hasManifest := reg.manifests["llm/tiny"]["q4"]
	reg.mu.Unlock()
	if !hasManifest {
